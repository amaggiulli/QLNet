...
namespace QLNet.Tests
{
   public class T_280
   {
      ...
      {
         var expectedYtm = 3.364514m;
         var expectedYtw = 3.236506m;

         var adapter = new QLNetAdapter(new NullLogger<QLNetAdapter>());
         var res = adapter.Calculate(basics, new DateTime(2019, 10, 04));
         var ytm = res.YieldToMaturity.Yield;
         var ytw = res.YieldToWorst.Yield;
         Assert.Equal(expectedYtm, ytm);
         Assert.Equal(expectedYtw, ytw);
      }
      ...
      public static IEnumerable<object[]> GetSecurityInfo()
      {
         yield return new object[]
         {
            new BondBasics
            {
               ...
               Price = 100.040m,
               Coupon = 4.75m,
               CouponType = CouponType.OIS,
               CouponTypeMethod = CouponTypeMethod.Unknown,
               MaturityDate = new DateTime(2019,10,15),
               AccrualDate = new DateTime(2002,02,15),
               FirstCouponDate = new DateTime(2002,10,15),
               NextCouponChangeDate = null,
               IssueDate = null,
               Call = new Redemption[]
               {
                  new Redemption(){Date = new DateTime(2019,10,14),Price = 100}
               },
               Put = null,
               Refund = null,
               EffectiveCall = null,
