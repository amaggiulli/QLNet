   ...
   public class T_Stats
   {
      ...
      public void testIncrementalStatistics()
      {
         ...
         if (stat.samples() != 500000)
            Assert.True(false, "stat.samples()  (" + stat.samples() + ") can not be reproduced against cached result (" + 500000 + ")");
         ...
         //   Assert.True(false, "variance (" + stat2.variance() + ") out of expected range " + 1E-2 + " +- " + tol);

      }

      private void TEST_INC_STAT(double expr, double expected)
      {
         if (!Utils.close_enough(expr, expected))
            Assert.True(false, " (" + expr + ") can not be reproduced against cached result (" + expected + ")");
      ...
      void check<S>(string name) where S : IGeneralStatistics, new ()
      {
         ...
         if (s.samples() != data.Length)
            Assert.True(false, name + ": wrong number of samples\n"
         ...
         if (calculated != expected)
            Assert.True(false, name + ": wrong sum of weights\n"
         ...
         calculated = s.min();
         if (calculated != expected)
            Assert.True(false, name + ": wrong minimum value\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
         ...
         calculated = s.max();
         if (calculated != expected)
            Assert.True(false, name + ": wrong maximum value\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
         ...
         if (Math.Abs(calculated - expected) > tolerance)
            Assert.True(false, name + ": wrong mean value\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
         ...
         if (Math.Abs(calculated - expected) > tolerance)
            Assert.True(false, name + ": wrong variance\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
         ...
         if (Math.Abs(calculated - expected) > tolerance)
            Assert.True(false, name + ": wrong standard deviation\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
         ...
         if (Math.Abs(calculated - expected) > tolerance)
            Assert.True(false, name + ": wrong skewness\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
         ...
         if (Math.Abs(calculated - expected) > tolerance)
            Assert.True(false, name + ": wrong kurtosis\n"
                         + "    calculated: " + calculated + "\n"
                         + "    expected:   " + expected);
      }
      ...
      void checkSequence<S>(string name, int dimension) where S : IGeneralStatistics, new ()
      {
         ...
         if (ss.samples() != data.Length)
            Assert.True(false, "SequenceStatistics<" + name + ">: "
      ...
      void checkConvergence<S>(string name) where S : IGeneralStatistics, new ()
      {
         ...
         if (calculatedSize != expectedSize1)
            Assert.True(false, "ConvergenceStatistics<" + name + ">: "
         ...
         if (Math.Abs(calculatedValue - expectedValue1) > tolerance)
            Assert.True(false, "wrong last value in convergence table"
         ...
         if (calculatedSamples != expectedSampleSize1)
            Assert.True(false, "wrong number of samples in convergence table"
         ...
         if (calculatedSize != expectedSize2)
            Assert.True(false, "wrong convergence-table size"
                         + "\n    calculated: " + calculatedSize
