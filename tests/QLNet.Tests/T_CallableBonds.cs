...
#pragma warning restore CS0612
   ...
   [Fact]
   public void testYieldToCallsOptimizedMatchesOriginal()
   {
      // Verify that the optimized version produces identical results to the original
      var settlementDate = new Date(27, 06, 2022);
      Settings.setEvaluationDate(settlementDate);
      var accrualDate = new Date(29,01,2020);
      var maturityDate = new Date(01, 07, 2050);
      ...
      var ccOriginal = callableBond.yieldToCalls(settlementDate, price, frequency, accuracy);
      var ccOptimized = callableBond.yieldToCallsOptimized(settlementDate, price, frequency, accuracy);

      // Debug output
      Console.WriteLine($"Original: {ccOriginal.Length} results");
      Console.WriteLine($"Optimized: {ccOptimized.Length} results");

      for (int i = 0; i < Math.Min(ccOriginal.Length, ccOptimized.Length); i++)
      {
         Console.WriteLine($"[{i}] Call Date: {ccOriginal[i].CallDate} vs {ccOptimized[i].CallDate}");
         Console.WriteLine($"[{i}] Call Price: {ccOriginal[i].CallPrice} vs {ccOptimized[i].CallPrice}");
         Console.WriteLine($"[{i}] Yield: {ccOriginal[i].CalcYield} vs {ccOptimized[i].CalcYield}");
         Console.WriteLine($"[{i}] Duration: {ccOriginal[i].CalcModifiedDuration} vs {ccOptimized[i].CalcModifiedDuration}");
      }

      // Verify they produce identical results
      QAssert.AreEqual(ccOriginal.Length, ccOptimized.Length);

      for (int i = 0; i < ccOriginal.Length; i++)
      {
         QAssert.AreEqual(ccOriginal[i].CallDate, ccOptimized[i].CallDate);
         QAssert.AreEqual(ccOriginal[i].CallPrice, ccOptimized[i].CallPrice);
         QAssert.AreEqual(ccOriginal[i].CalcYield, ccOptimized[i].CalcYield);
         QAssert.AreEqual(ccOriginal[i].CalcModifiedDuration, ccOptimized[i].CalcModifiedDuration);
         QAssert.AreEqual(ccOriginal[i].ErrorMessage, ccOptimized[i].ErrorMessage);
      }
   }

   [Fact]
   public void testYieldToCallsOptimizedDoesNotModifyOriginalBond()
   {
      // Verify that the optimized method does not modify the original bond
      var settlementDate = new Date(27, 06, 2022);
      Settings.setEvaluationDate(settlementDate);
      var accrualDate = new Date(29,01,2020);
      var maturityDate = new Date(01, 07, 2050);
      ...
      var originalCashflowCount = callableBond.cashflows().Count;
      var originalMaturityDate = callableBond.maturityDate();
      var originalAccruedAmount = callableBond.accruedAmount(settlementDate);
      var originalNotional = callableBond.notional(settlementDate);

      // Serialize original cashflows for comparison
      var originalCashflowDates = callableBond.cashflows().Select(cf => cf.date()).ToList();
      var originalCashflowAmounts = callableBond.cashflows().Select(cf => cf.amount()).ToList();

      // Call optimized method multiple times
      var ccOptimized1 = callableBond.yieldToCallsOptimized(settlementDate, price, frequency, accuracy);
      var ccOptimized2 = callableBond.yieldToCallsOptimized(settlementDate, price, frequency, accuracy);
      var ccOptimized3 = callableBond.yieldToCallsOptimized(settlementDate, price, frequency, accuracy);

      // Verify bond state is unchanged
      QAssert.AreEqual(originalCashflowCount, callableBond.cashflows().Count);
      QAssert.AreEqual(originalMaturityDate, callableBond.maturityDate());
      QAssert.AreEqual(originalAccruedAmount, callableBond.accruedAmount(settlementDate));
      QAssert.AreEqual(originalNotional, callableBond.notional(settlementDate));

      // Verify cashflows are unchanged
      var currentCashflowDates = callableBond.cashflows().Select(cf => cf.date()).ToList();
      var currentCashflowAmounts = callableBond.cashflows().Select(cf => cf.amount()).ToList();

      for (int i = 0; i < originalCashflowCount; i++)
      {
         QAssert.AreEqual(originalCashflowDates[i], currentCashflowDates[i]);
         QAssert.AreEqual(originalCashflowAmounts[i], currentCashflowAmounts[i]);
      }

      // Verify results are consistent across multiple calls
      QAssert.AreEqual(ccOptimized1.Length, ccOptimized2.Length);
      QAssert.AreEqual(ccOptimized1.Length, ccOptimized3.Length);

      for (int i = 0; i < ccOptimized1.Length; i++)
      {
         QAssert.AreEqual(ccOptimized1[i].CalcYield, ccOptimized2[i].CalcYield);
         QAssert.AreEqual(ccOptimized1[i].CalcYield, ccOptimized3[i].CalcYield);
         QAssert.AreEqual(ccOptimized1[i].CalcModifiedDuration, ccOptimized2[i].CalcModifiedDuration);
         QAssert.AreEqual(ccOptimized1[i].CalcModifiedDuration, ccOptimized3[i].CalcModifiedDuration);
      }
   }
