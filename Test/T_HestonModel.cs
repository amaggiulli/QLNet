   ...
   public class T_HestonModel
   {
      ...
      public void testBlackCalibration()
      {
         ...
         using (SavedSettings backup = new SavedSettings())
         {
            ...
            for (double sigma = 0.1; sigma < 0.7; sigma += 0.2)
            {
               ...
               {
                  Assert.Fail("Failed to reproduce expected sigma"
               ...
               {
                  Assert.Fail("Failed to reproduce expected theta"
               ...
               {
                  Assert.Fail("Failed to reproduce expected v0"
                              + "\n    calculated: " + model.v0()
                              + "\n    expected:   " + volatility*volatility);
               }
            }
      ...
      public void testDAXCalibration()
      {
         ...
         using (SavedSettings backup = new SavedSettings())
         {
            ...
            {
               Assert.Fail("Failed to reproduce calibration error"
                          + "\n    calculated: " + sse
                          + "\n    expected:   " + expected);
            }
         }
      ...
      public void testAnalyticVsBlack()
      {
         ...
         //if (error > tolerance)
         //{
         //   Assert.Fail("failed to reproduce Black price with AnalyticHestonEngine"
         ...
         //if (error > tolerance)
         //{
         //   Assert.Fail("failed to reproduce Black price with FdHestonVanillaEngine"
         //              +"\n    calculated: " + calculated
         //              +"\n    expected:   " + expected
         //              +"\n    error:      " + error);
         //}
      ...
      public void testAnalyticVsCached()
      {
         ...
         using (SavedSettings backup = new SavedSettings())
         {
            ...
            {
               Assert.Fail("Failed to reproduce cached analytic price"
            ...
            for (i = 0; i < 3; ++i)
            {
               ...
               {
                  Assert.Fail("Failed to reproduce cached analytic prices:"
      ...
      public void testMcVsCached()
      {
         ...
         using (SavedSettings backup = new SavedSettings())
         {
            ...
            {
               Assert.Fail("failed to reproduce error estimate"
      ...
      //   if (error > 1.0e-3)
      //   {
      //      Assert.Fail("failed to reproduce cached price with FD Barrier engine"
      //                 + "\n    calculated: " + calculated
      //                 + "\n    expected:   " + expected
      //                 + "\n    error:      " +  error);
      //   }
      ...
      //   if (error > tolerance)
      //   {
      //      Assert.Fail("failed to reproduce cached price with FD engine"
      //                 + "\n    calculated: " + calculated
      //                 + "\n    expected:   " + expected
      //                 + "\n    error:      " +  error);
      //   }
      ...
      //   if (error > tolerance)
      //   {
      //      Assert.Fail("failed to reproduce discrete dividend price with FD engine"
      //                 + "\n    calculated: " + calculated
      //                 + "\n    expected:   " + expected
      //                 + "\n    error:      " +  error);
      //   }
      ...
      //   if (error > tolerance)
      //   {
      //      Assert.Fail("failed to reproduce american option price with FD engine"
      //                 + "\n    calculated: " + calculated
      //                 + "\n    expected:   " + expected
      //                 + "\n    error:      " + error);
      //   }
      ...
      public void testKahlJaeckelCase()
      {
         ...
         //   if (Math.Abs(calculated - expected) > 2.34*errorEstimate)
         //   {
         //      Assert.Fail("Failed to reproduce cached price with MC engine"
         ...
         //   if (errorEstimate > tolerance)
         //   {
         //      Assert.Fail("failed to reproduce error estimate with MC engine"
         //                  + "\n    discretization: " + descriptions[i].name
         //                  + "\n    calculated    : " + errorEstimate
         //                  + "\n    expected      :   " + tolerance);
         //   }
      ...
      public void testDifferentIntegrals()
      {
         ...
         using (SavedSettings backup = new SavedSettings())
         {
            ...
            foreach (var iter in parameters)
            {
               ...
               if (maxDiff > tr)
               {
                  Assert.Fail("Failed to reproduce Heston pricing values within given tolerance"
      ...
      public void testMultipleStrikesEngine()
      {
         ...
         //   {
         //      Assert.Fail("failed to reproduce price with FD multi strike engine"
         ...
         //   {
         //      Assert.Fail("failed to reproduce delta with FD multi strike engine"
         ...
         //   {
         //      Assert.Fail("failed to reproduce gamma with FD multi strike engine"
         ...
         //   {
         //      Assert.Fail( "failed to reproduce theta with FD multi strike engine"
      ...
      public void testAnalyticPiecewiseTimeDependent()
      {
         ...
         using (SavedSettings backup = new SavedSettings())
         {
            ...
            {
               Assert.Fail("failed to reproduce heston prices "
                           + "\n    calculated: " + calculated
                           + "\n    expected:   " + expected);
            }
         }
            ...
            for (int i = 0; i < strikes.Length; ++i)
            {
               ...
               for (int j = 0; j < types.Length; ++j)
               {
                  ...
                  for (int k = 0; k < engines.Length; ++k)
                  {
                     ...
                     if (relError > tol)
                     {
                        Assert.Fail("failed to reproduce Alan Lewis Reference prices "
                                 + "\n    strike     : " + strike
                                 + "\n    option type: " + type
                                 + "\n    engine type: " + k
                                 + "\n    rel. error : " + relError);
            ...
            for (int j = 0; j < terms.Length; ++j)
            {
               ...
               for (int i = 0; i < strikes.Length; ++i)
               {
                  ...
                  for (int k = 0; k < expansions.Length; ++k)
                  {
                     ...
                     if (relError > refTol)
                     {
                        Assert.Fail( "failed to reproduce Forde reference vols "
                                  + "\n    strike        : " + strike
                                  + "\n    expansion type: " + k
                                  + "\n    rel. error    : " + relError);
                     }
