...
namespace QLNet.Termstructures.Inflation
{
    public class InflationFittingMethods : FittedInflationBondCurve.FittingInflationMethod
    {
        ...
        public double LinearFitValue(Vector x, double t)
        {
            ...
            {
                y[i] = x[i] * _initialCurveData[i].rate;
                diff[i] = (x[i + 1] * _initialCurveData[i + 1].rate - x[i] * _initialCurveData[i].rate) / (_tenors[i + 1] - _tenors[i]);
            }
        ...
        public override ZeroIndex GetCurve(Vector x)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < size_; ++i)
                {
                    ...
                    zciisData[i].rate = LinearFitValue(x, _initialCurveData[i].tenor);
