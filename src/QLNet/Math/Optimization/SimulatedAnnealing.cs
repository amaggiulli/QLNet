...
/* 
     Numerical Recipes in C (second edition), Chapter 10.9,
...
namespace QLNet
{
    ...
    */
    public class SimulatedAnnealing<RNG> : OptimizationMethod where RNG : class, IRNGTraits, new()
    {
        ...
        public SimulatedAnnealing(double lambda, double T0,
        ...
        /* budget a total of K moves, set temperature T to the initial
          ...
          algorithm.
         */
        public SimulatedAnnealing(double lambda, double T0, int K,
                           double alpha, RNG rng = default(RNG))
        {
        ...
        override public EndCriteria.Type minimize(Problem P, EndCriteria endCriteria)
        {
            ...
            for (i_ = 0; i_ <= n_; i_++) {
                ...
                if (Double.IsNaN(ytry_)) { // handle NAN
                    values_[i_] = Double.MaxValue;
                }
            }
