    ...
    */
    ...
    public class DifferentialEvolution : OptimizationMethod
    {
        ...
        public class Candidate : ICloneable
        {
            ...
            public object Clone()
            {
                Candidate c = new Candidate(this.values.size());
                this.values.ForEach((ii, vv) => c.values[ii] = vv);
                c.cost = this.cost;
                return c;
            }
        }
        ...
        public override EndCriteria.Type minimize(Problem P,
                                                  EndCriteria endCriteria)
        {
            ...
            while (!endCriteria.checkMaxIterations(iteration++, ref ecType)) {
                ...
                if (fxNew < bestMemberEver_.cost && P.constraint().test(tmp.values))
                    bestMemberEver_ = tmp;
        ...
        protected void fillInitialPopulation(List<Candidate> population,
                                   Problem p)
        {
                ...
                for (int coIter = 0;coIter< currGenCrossover_.size(); coIter++){
                    mutationProbabilities[coIter] =
                        (1.0 -Math.Pow(currGenCrossover_[coIter],
                                        (int) population.First().values.size()))
                                               ...
                                               CostFunction costFunction)
        {
            ...
            switch (configuration().strategy) {
              ...
              case Strategy.BestMemberWithJitter: {
                  ...
                  for (int popIter = 0; popIter < population.Count; popIter++) {
                      ...
                      population[popIter].values = bestMemberEver_.values
                          + Vector.DirectMultiply(shuffledPop1[popIter].values - population[popIter].values
              ...
              case Strategy.Rand1DiffWithPerVectorDither: {
                  ...
                  for (int popIter = 0; popIter < population.Count; popIter++) {
                      population[popIter].values = population[popIter].values
                          + Vector.DirectMultiply(FWeight, shuffledPop1[popIter].values - shuffledPop2[popIter].values);
                  }
              }
                break;
                       ...
                       CostFunction costFunction)
        {
            ...
            for (int popIter = 0; popIter < population.Count; popIter++) {
                population[popIter].values = Vector.DirectMultiply(oldPopulation[popIter].values, invCrossoverMask[popIter])
                                           + Vector.DirectMultiply(mutantPopulation[popIter].values, crossoverMask[popIter]);
