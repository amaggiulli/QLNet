...
namespace QLNet
{
    ...
    public class Concentrating1dMesher : Fdm1dMesher
    {
            ...
            : base(size)
        {
            ...
            double? density = cPoints.second ==null ?
                              null : cPoints.second*(end - start);
            ...
            if (cPoint != null) {
                ...
                double c1 = Utils.asinh((start - cPoint.Value) / density.Value);
                double c2 = Utils.asinh((end - cPoint.Value) / density.Value);
                if (requireCPoint) {
                    u.Add(0.0);
                    z.Add(0.0);
                ...
                for (int i = 1; i < size - 1; ++i) {
                    ...
                    locations_[i] = cPoint.Value
                        + density.Value*Math.Sinh(c1*(1.0 - li) + c2*li);
                }
            }
            else {
                for (int i = 1; i < size - 1; ++i) {
        ...
        public class OdeIntegrationFct {
            ...
            protected double jac(double a, double x, double y) {
                ...
                for (int i=0; i < points_.Count; ++i) {
                    s+=1.0/(betas_[i].Value + (y - points_[i].Value) * (y - points_[i].Value));
                }
            ...
            : base(size)
        {
            ...
            for (int i=0; i < points.Count; ++i) {
                double c1 = Utils.asinh((start-points[i].Value)/betas[i].Value);
                double c2 = Utils.asinh((end-points[i].Value)/betas[i].Value);
            ...
            List<Pair<double?, double?>> w = new InitializedList<Pair<double?,double?>>(1, new Pair<double?, double?>(0.0,0.0));
