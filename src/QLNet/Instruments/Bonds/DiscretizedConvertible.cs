...
namespace QLNet
{
    ...
    public class DiscretizedConvertible : DiscretizedAsset
    {
        ...
        public ConvertibleBond.option.arguments arguments() { return arguments_; }
        public GeneralizedBlackScholesProcess process() { return process_; }

        public DiscretizedConvertible(ConvertibleBond.option.arguments args, GeneralizedBlackScholesProcess process, TimeGrid grid)
        {
            arguments_ = args;
            process_ = process;
            ...
            Date settlementDate = process.riskFreeRate().currentLink().referenceDate();
            for (var i = 0; i < arguments_.dividends.Count(); i++)
            {
                if (arguments_.dividends[i].date() >= settlementDate)
                {
                    dividendValues_[i] = arguments_.dividends[i].amount() * process.riskFreeRate().currentLink().discount(arguments_.dividends[i].date());
                }
            }
        ...
        public override void reset(int size)
        {
            ...
            // values_ = array(size,arguments_cashFlows.back()->amount());
            ...
            InterestRate riskFreeRate = process_.riskFreeRate().currentLink().zeroRate(exercise, rfdc, Compounding.Continuous, Frequency.NoFrequency);


            // Claculate blended discount rate to be used on roll back .
            for (var j = 0; j < values_.Count(); j++)
            {
                spreadAdjustedRate_[j] = conversionProbability_[j] * riskFreeRate.value() + (1 - conversionProbability_[j]) * (riskFreeRate.value() + creditSpread.currentLink().value());
            }
        }
                    ...
                    if (arguments_.callabilityTriggers[i] != null)
                    {
                        ...
                        // the callability is conditioned by the trigger ...
                        {
                            if (grid[j] >= trigger)
                            {
                                ...
                                values_[j] = Math.Min(Math.Max(arguments_.callabilityPrices[i], Convert.ToDouble(arguments_.conversionRatio) * grid[j]), values_[j]);
                            }
                        }
                    }
                    else if (convertible)
        ...
        public Vector adjustedGrid()
        {
            ...
            for (var i = 0; i < arguments_.dividends.Count(); i++)
            {
                ...
                if (dividendTime >= t || Utils.close(dividendTime, t))
                {
                    ...
                    double dividendDiscount = (process_.riskFreeRate().currentLink().discount(dividendTime)) / (process_.riskFreeRate().currentLink().discount(t));
